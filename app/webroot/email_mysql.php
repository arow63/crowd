...
while ($row = mysql_fetch_assoc($result)) {
	...
	$sql = "UPDATE users SET email = 'demo+".$newuser."@arow63.com' WHERE users.id=".$row['id'];  
...
foreach($row as $row1 ){
   $result = pg_query($db,"UPDATE users SET email = 'demo+".$row1['username'].$row1['id']."@arow63.com' WHERE users.id=".$row1['id']); 
   if (!$result){
    echo "Update failed!!";
    }
    else
